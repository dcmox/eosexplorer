...
export const splitStringByCSVCommas = (str: string, replaceEmptyColumnWithIndex: boolean = false) => {
    if (str.charAt(0) === ',') {
    ...
    return results.map((column: string, index: number) =>
...
export const mergeKVArrays = (keys: any[], values: any[]): object =>
    keys.reduce((obj: object, key: string, index: number) => ({ ...obj, [key]: values[index] }), {})

export const csvToJson = (csv: string) => {
    let csvData: string[] | object[] = csv.split('\n').filter((line: string) => line) // remove empty lines

    const csvHeaderRow: string|undefined = csvData.shift()
    const result: ICsvToJsonResult = { csv: [] }
    if (csvHeaderRow) {
        const headers: Array<string | number | null> = splitStringByCSVCommas(csvHeaderRow, true)
        csvData = csvData.map((row: string) => mergeKVArrays(headers, splitStringByCSVCommas(row)))
        result.csv = csvData
    }
